...
Changesets.prototype.saveDefaults = function($inputs) {
  if ($inputs == undefined) {
    $inputs = $('input');
  }
...
Changesets.prototype.loadDefaults = function($inputs) {
  $inputs.each(function() {
    var $this = $(this);
...
Changesets.prototype.setupBindings = function() {  
  $('article')
...
Changesets.prototype.handleSaveNew = function($thisRow, $loading, data) {
  return function(xhr) {
...
Changesets.prototype.handleSave = function($thisRow, $thatRow, $loading, data) {
  return function(xhr) {
    if (xhr.status == 422) {
...
Changesets.prototype.handleDelete = function($t) {
  $t.add($t.next('article')).fadeOut(750, function() {
    $(this).remove();
  });
}

Changesets.prototype.handleToggle = function(e, isCancel) {
  var $thisRow = $(e.target).closest('article'),
      $thatRow;
...
Changesets.prototype.getRecordID = function($t) {
  var csID = $t.closest('article').attr('id');
  return csID.substring(csID.lastIndexOf('-') + 1);
}

Changesets.prototype.flash = function(message, isError) {
...
Changesets.prototype.attachBindings = function() {
